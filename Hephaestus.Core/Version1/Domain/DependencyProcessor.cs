...
namespace Hephaestus.Core.Version1.Domain
{
    ...
    {
        private readonly INamespaceLookup _namespaceLookup; //Dictionary<string, List<Project>> namespaceLookup
        ...
        public void ProcessDirectDependencies(ProjectV1 project)
        {
            ...
            var directPackageDependencies = new List<PackageReferenceV1>();
            var directProjectDependencies = new List<ProjectReferenceV1>();
            ...
            if (_testProjectPredicate.IsTestProject(project.ProjectName))
            {
                ...
                {
                    var projectReferences = _namespaceLookup.GetProjectsForNamespace(subjectNamespace, null);
        ...
        private void AddReferencesForNamespace(
            ...
            Dictionary<string, ProjectReferenceV1> existingProjectDependencies,
            Dictionary<string, PackageReferenceV1> existingPackageDependencies,
            List<ProjectReferenceV1> directProjectDependencies,
            List<PackageReferenceV1> directPackageDependencies)
        {
            var projectReferences = _namespaceLookup.GetProjectsForNamespace(usingDirective, null).ToArray();
            var packageReferences = _namespaceLookup.GetPackageReferencesForNamespace(usingDirective, null).ToArray();
        ...
        private void AddMandatoryNonDirectDependencies(ProjectV1 project, List<PackageReferenceV1> packages, List<ProjectReferenceV1> projects)
        {
