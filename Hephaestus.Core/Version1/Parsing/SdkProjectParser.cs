...
{
    internal class SdkProjectParser : IProjectParserV1
    {
        ...
        public SdkProjectParser(XDocument content, string projectPath, IFileProvider fileProvider)
        {
            _content = content;
        ...
        {
            return new OutputTypeTranslator().Translate(_content.Descendants("OutputType").SingleOrDefault()?.Value);
        }
        ...
        public Framework ParseTargetFrameworkMoniker()
        {
            ...
            {
                return new TfmTranslator().Translate(_content.Descendants("TargetFrameworks").SingleOrDefault()?.Value.Split(",")
                    .First());
                //TODO Not Ideal only takes the first in a collection.  Need a better way.
            }

            return new TfmTranslator().Translate(_content.Descendants("TargetFramework").SingleOrDefault()?.Value);
        }
        ...
        public ICollection<EmbeddedResourceV1> ParseEmbeddedResources()
        {
            return _content.Descendants("EmbeddedResource")
                .Select(x =>
                {
                    ...
                    if (include == null) return new EmbeddedResourceV1("Unknown Embedded Resource", null);
        ...
        public ICollection<PackageReferenceV1> ParsePackageReferences()
        {
            return _content.Descendants("ItemGroup").Descendants("PackageReference")
                .Select(x =>
                {
                    ...
                    return new PackageReferenceV1(name, version);
                }).ToArray();
        }
